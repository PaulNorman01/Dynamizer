...
#include "AnnoyersInclude.hpp"

using std::cout;

DWORD g_origCrc = 0x0000; //Change ME!
extern BOOL g_bDebug = true;

#define WALKER StackWalker::WalkerOp spoof(_AddressOfReturnAddress());
#define _CRT_SECURE_NO_WARNINGS_
#define BUFFER_SIZE 100
#define SHELL_SIZE 500
//#define SPOOF_CALL(name) (StackWalker::PtrRotate(name))


void DebuggeeFunction(); //CHANGE ME!
...
namespace StackWalker
{
	...
	public:
		uintptr_t tmpVal = 0;
		const uintptr_t KVal = 0xff00ff00ff00ff00;
		void* ReturnAddr = 0;

		WalkerOp(void* addr) :ReturnAddr(addr)
		{
			tmpVal = *(uintptr_t*)ReturnAddr;
			tmpVal ^= KVal;
		...
		{
			tmpVal ^= KVal;
			*(uintptr_t*)ReturnAddr = tmpVal;
		}
	};



	__forceinline PVOID LocateShellCode(PVOID func, size_t size = SHELL_SIZE)
	{
		...
		__declspec(safebuffers)ShellCodeGenerator(Func f, Args&... args)
	{
		...
		const uintptr_t KVal = 0xfa23f3519f22fb10;
		void* ReturnAddr = _AddressOfReturnAddress();
		uintptr_t tmpVal = *(uintptr_t*)ReturnAddr;
		tmpVal ^= KVal;
		*(uintptr_t*)ReturnAddr = 0;
	...
	public:
		PtrRotate(Func* func) :funcPtr(func) {}
		...
		__forceinline decltype(auto) operator()(Args&&... args)
		{
			...
			static size_t count{};
			static p_shell_code_generator_type orig_generator[BUFFER_SIZE]{};
			static p_shell_code_generator_type alloc_generator[BUFFER_SIZE]{};
...
namespace skc
{
    ...
    {
    public:
        __forceinline constexpr StringKrypter(T* data)
        {
            crypt(data);
        }
...
#define StringKrypt(str) StringKrypt_key(str, __TIME__[4], __TIME__[7])
#define StringKrypt_key(str, key1, key2) []() { \
			constexpr static auto crypted = skc::StringKrypter \
...
void GoThroughAllAnnoyers(HANDLE thrd)
{
	EventCtrlFunction();
	handleException();
	StopHardwareDebugger(thrd);
	AntiStep(); //USE ME THROUGH FUNCTIONS. 
}
